...
import { UploadCloud , PlusIcon, UploadCloudIcon ,FilePenLineIcon ,TrashIcon, PencilIcon, XIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  try {
    const {data} = await api.get('/api/users/resumes', {title}, {headers: { Authorization: `${token}`}})
  ...
  const createResume = async (e) => {
    ...
    // Implementation for creating a new resume
  }
  ...
  const uploadResume = async (e) => {
    ...
    try {
      const resumeText = await pdfToText(resume)
        ...
        console.log("Selected file:", resume);        // 👈 check file exists
  ...
  return (
    ...
    <div className='max-w-7xl mx-auto px-4 py-8'>
      <p className='text-2xl font-medium mb-6 bg-gradient-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'>Welcome, Shikha Sahu</p>
